...
class Test10ViewController: UIViewController {
    ...
    let quiznumLabel = UILabel().then {
        let fullString = NSMutableAttributedString(string: "Q.10", attributes: [
    ...
    let hintLabel2 = UILabel().then {
        ...
        let attrString = NSMutableAttributedString(string: "ex. 이걸 좋다고 해야 돼.. 싫다고 해야 돼.. 진짜 웃안웃이다.", attributes: [NSAttributedString.Key.paragraphStyle: paragraphStyle])
        $0.attributedText = attrString
    }
    ...
    @objc func didTapnextButton(_ sender: Any) {
        let addVC = TestEndViewController()
        addVC.modalPresentationStyle = .overFullScreen
