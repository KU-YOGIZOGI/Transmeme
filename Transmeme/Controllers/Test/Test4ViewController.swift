...
class Test4ViewController: UIViewController {
    ...
    let quiznumLabel = UILabel().then {
        let fullString = NSMutableAttributedString(string: "Q.4", attributes: [
    ...
    let hintLabel2 = UILabel().then {
        ...
        let attrString = NSMutableAttributedString(string: "ex. 오늘은 꾸안꾸 스타일로 입어야지~", attributes: [NSAttributedString.Key.paragraphStyle: paragraphStyle])
        $0.attributedText = attrString
    }
