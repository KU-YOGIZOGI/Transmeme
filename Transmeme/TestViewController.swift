...
class TestViewController: UIViewController {
    ...
    let quiznumLabel = UILabel().then {
        let fullString = NSMutableAttributedString(string: "Q.15", attributes: [
    ...
    let quizButton = UIButton().then {
        $0.setImage(UIImage(named: "quizButtonn"), for: .normal)
    }
    let quizLabel = UILabel().then {
        $0.text = "자신있게 만남 추구"
        $0.textColor = UIColor(red: 79/255.0, green: 79/255.0, blue: 79/255.0, alpha: 1.0)
        $0.font = UIFont(name: "GmarketSansMedium", size: 18)
        $0.numberOfLines = 0
    }
    let quizselect = UIImageView().then {
        $0.image = UIImage(named: "quizncircle")
    }
